...
def test_parser():
    ...
    parser.add_argument('--save_npy', action='store_true',
                        help='whether to save prediction and gt result'
                             'in npy file')
    parser.add_argument('--lidar_ratio', type=float, default=0.5,)
...
def main():
    ...
    with open(os.path.join(opt.model_dir, 'result_{}.txt'.format(opt.result_name)), 'a+') as f:
        f.write('ap30: {:.04f} ap50: {:.04f} ap70: {:.04f} comm_thre: {:.04f} comm_rate: {:.06f} ratio: {}\n' .format(ap30, ap50, ap70, opt.comm_thre, comm_rates, opt.lidar_ratio))
