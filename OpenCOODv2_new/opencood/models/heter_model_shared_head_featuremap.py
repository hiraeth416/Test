...
import torchvision
from opencood.models.sub_modules.codebook_test import ChannelCompressor
from opencood.models.sub_modules.codebook_test import UMGMQuantizer

class HeterModelSharedheadFeatureMap(nn.Module):
    def __init__(self, args):
        super(HeterModelSharedheadFeatureMap, self).__init__()
        self.args = args
        self.vis_count = 0
        #self.log = args['heatmap_name']
        self.save_path_before = args['codebook']['save_path_before']
        self.save_path_after = args['codebook']['save_path_after']
        self.save_path_fused = args['codebook']['save_path_fused']
    ...
    def forward(self, data_dict):
        ...
        if DEBUG:
            ...
            #save_dir = os.path.join(self.save_path_before, '/i{}_agent{}.png'.format(self.vis_count, self.vis_count))
            N = heter_feature_2d.shape[0]
            #print("N:", N)
            #fig, axes = plt.subplots(N, 1)
            # for ax_i, cur_feat in enumerate(split_feat_list_single[0][:,0]):
            for ax_i, cur_feat in enumerate(heter_feature_2d):
                fig, axes = plt.subplots(1, 1, figsize=(35.2, 10))
                save_dir = os.path.join(self.save_path_before, 'i%d_agent%d.png' % (self.vis_count, ax_i))
                #print(save_dir)
                cur_feat = cur_feat.max(dim=0)[0].cpu().numpy()
                sns.heatmap(cur_feat, ax=axes, vmin=y_min, vmax=y_max, cbar=True)
                axes.set_xticks([])
                axes.set_yticks([])
                axes.set_title('agent_{}_before_codebook'.format(ax_i))
                fig.tight_layout()
                plt.savefig(save_dir)
                plt.close()

            '''
            cur_feat = heter_feature_2d[0].max(dim=0)[0].cpu().numpy()
        ...
        if self.multi_channel_compressor_flag:
            ...
            heter_feature_2d, codebook_code, codebook_logit, codebook_loss = self.multi_channel_compressor(heter_feature_2d)
            #print("heter_feature_2d_shape: ", heter_feature_2d.shape)
            ...
            #print(len(codebook_code))
            codebook_code=torch.tensor([item.cpu().detach().numpy() for item in codebook_code]).cuda()
            #print("codebook_code_shape: ", codebook_code.shape)
            #codebook_code=np.array(codebook_code)       
            codebook_code = codebook_code.view(-1, H, W, 1).permute(0, 3, 1, 2).contiguous()
            #print("codebook_code_shape: ", codebook_code.shape)
            #print(codebook_code)
            shape_num = 0
            ...
            """
            if DEBUG:
                save_dir = '/GPFS/rhome/sifeiliu/OpenCOODv2_new/opencood/logs_HEAL/vis_feature_map_{}/{}_index.png'.format(self.log, self.vis_count)
                N = codebook_code.shape[0]
                figsize=(30,30)
                fig, axes = plt.subplots(N, 1)
                # for ax_i, cur_feat in enumerate(split_feat_list_single[0][:,0]):
                '''
                for ax_i, cur_feat in enumerate(heter_feature_2d):
                    ...
                    axes[ax_i].set_title('agent_{}_after'.format(ax_i))
                '''
            
                if N > 1:
                    for ax_i, cur_feat in enumerate(codebook_code):
                        cur_feat = cur_feat.max(dim=0)[0].cpu().numpy()
                        sns.heatmap(cur_feat, ax=axes[ax_i], vmin=0, vmax=255, cbar=True)
                        axes[ax_i].set_xticks([])
                        axes[ax_i].set_yticks([])
                        axes[ax_i].set_title('agent_{}_index'.format(ax_i))
                else:
                    cur_feat = codebook_code[0].max(dim=0)[0].cpu().numpy()
                    sns.heatmap(cur_feat, ax=axes, vmin=0, vmax=255, cbar=True)
                    axes.set_xticks([])
                    axes.set_yticks([])
                    axes.set_title('agent_0_index')
                '''
                cur_feat = codebook_code[0].max(dim=0)[0].cpu().numpy()
                sns.heatmap(cur_feat, ax=axes[-1], cbar=True)
                axes[-1].set_xticks([])
        ...
        if DEBUG:
            ...
            # use_mask = True
            # mask = ((cls.sigmoid()[0][0] > 0.1)*1.0).cpu().numpy()
            #save_dir = '/GPFS/rhome/sifeiliu/OpenCOODv2_new/opencood/logs_HEAL/vis_feature_map_{}/{}_after.png'.format(self.log, self.vis_count)
            N = heter_feature_2d.shape[0]
            #fig, axes = plt.subplots(N, 1)
            # for ax_i, cur_feat in enumerate(split_feat_list_single[0][:,0]):
            '''
            ...
            for ax_i, cur_feat in enumerate(heter_feature_2d):
                fig, axes = plt.subplots(1, 1, figsize=(35.2, 10))
                save_dir = os.path.join(self.save_path_after,  'i%d_agent%d.png' % (self.vis_count, ax_i))
                cur_feat = cur_feat.max(dim=0)[0].cpu().numpy()
                sns.heatmap(cur_feat, ax=axes, vmin=y_min, vmax=y_max, cbar=True)
                axes.set_xticks([])
                axes.set_yticks([])
                axes.set_title('agent_{}_after_codebook'.format(ax_i))
                fig.tight_layout()
                plt.savefig(save_dir)
                plt.close()
            '''
        ...
        if DEBUG:
            ...
            """
            for ax_i, cur_feat in enumerate(fused_feature_list[0]):
                fig, axes = plt.subplots(1, 1, figsize=(35.2, 10))
                save_dir = os.path.join(self.save_path_fused,  'i%d.png' % (self.vis_count))
                cur_feat = cur_feat.max(dim=0)[0].cpu().numpy()
                sns.heatmap(cur_feat, ax=axes, vmin=y_min, vmax=y_max, cbar=True)
                axes.set_xticks([])
                axes.set_yticks([])
                axes.set_title('agent_{}_fused_feature'.format(ax_i))
                fig.tight_layout()
                plt.savefig(save_dir)
                plt.close()
            """
            fig, axes = plt.subplots(1, 1, figsize=(35.2, 10))
            save_dir = os.path.join(self.save_path_fused,  'i%d.png' % (self.vis_count))
            cur_feat = fused_feature_list[0][0].max(dim=0)[0].cpu().numpy()
            sns.heatmap(cur_feat, ax=axes, vmin=y_min, vmax=y_max, cbar=True)
            axes.set_xticks([])
            axes.set_yticks([])
            axes.set_title('fused')
            fig.tight_layout()
            plt.savefig(save_dir)
            plt.close()
