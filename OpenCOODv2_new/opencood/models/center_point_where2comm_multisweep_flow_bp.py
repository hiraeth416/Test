...
class CenterPointWhere2commMultiSweepFlow(nn.Module):
    ...
    def iterate(self, cur_feat_list, lidar_pose, record_len, pairwise_t_matrix, prev_confidence_maps=None, hist_fused_feats=None, flow_gt=None, fuse_layer_id=0):
        ...
        if hist_fused_feats is not None:
            ...
            # temporal fusion
            # feats = self.temporal_fusion(cur_feat_list[fuse_layer_id], compensated_fused_feat)
            # feats = hist_fused_feats[-1]
            feats = compensated_fused_feat
        else:
    ...
    def forward(self, data_dict):
        ...
        for layer_id, feat in enumerate(split_feat_list_single):
            if layer_id == fuse_layer_id:
                feat_list_colla.append(self.get_ego_feat(fused_feat, record_len))
