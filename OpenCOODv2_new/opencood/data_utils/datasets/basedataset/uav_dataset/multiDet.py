...
import cv2
import os
from .transformation import get_crop_shift_mat, get_shift_coord
from .images import flip, color_aug
from .images import get_affine_transform, affine_transform
from .images import gaussian_radius, draw_umich_gaussian, draw_msra_gaussian, draw_polygon_gaussian
from .images import draw_dense_reg
import math
import kornia
import copy
...
class MultiAgentDetDataset(data.Dataset):
    ...
    def load_sample(self, sample, img_dir, num_images=8):
                ...
                if cam in cam_list:
                    ...
                    if self.opt['noise'] > 0:
                        trans_mats_list_withnoise.append(
                            info['trans_mat_withnoise{:01d}'.format(int(self.opt['noise'] * 10))])
                        shift_mats_list_withnoise.append(
                            info['shift_mats_withnoise'][self.opt['noise']][2 ** self.opt['trans_layer'][-1]])
                    else:
        ...
        draw_gaussian_i = draw_msra_gaussian if self.opt['mse_loss'] is True else \
            draw_umich_gaussian
        ...
        if self.opt['real']:
            ...
            # output_h_bev, output_w_bev = int(96 / scale), int(192 / scale)
        ...
        trans_mats_n010[:min(num_images, len(trans_mats_list))] = trans_mats_list_n010[
                                                                  :min(num_images, len(trans_mats_list))]
        trans_mats_n005[:min(num_images, len(trans_mats_list))] = trans_mats_list_n005[
                                                                  :min(num_images, len(trans_mats_list))]
        trans_mats_p005[:min(num_images, len(trans_mats_list))] = trans_mats_list_p005[
                                                                  :min(num_images, len(trans_mats_list))]
        trans_mats_p007[:min(num_images, len(trans_mats_list))] = trans_mats_list_p007[
                                                                  :min(num_images, len(trans_mats_list))]
        trans_mats_p010[:min(num_images, len(trans_mats_list))] = trans_mats_list_p010[
                                                                  :min(num_images, len(trans_mats_list))]
        trans_mats_p015[:min(num_images, len(trans_mats_list))] = trans_mats_list_p015[
                                                                  :min(num_images, len(trans_mats_list))]
        trans_mats_p020[:min(num_images, len(trans_mats_list))] = trans_mats_list_p020[
                                                                  :min(num_images, len(trans_mats_list))]
        trans_mats_p080[:min(num_images, len(trans_mats_list))] = trans_mats_list_p080[
                                                                  :min(num_images, len(trans_mats_list))]
        ...
        trans_mats_withnoise[:min(num_images, len(trans_mats_list))] = trans_mats_list_withnoise[
                                                                       :min(num_images, len(trans_mats_list))]
        shift_mats_withnoise[:min(num_images, len(trans_mats_list))] = shift_mats_list_withnoise[
                                                                       :min(num_images, len(shift_mats_8))]
        ...
        for index, info in enumerate(zip(images, vehicles, vehicles_i, vehicles_z, category_idx)):
            ...
            if self.split == 'train' and self.opt['no_color_aug'] is False:
            ...
            for k in range(num_objs):
                ...
                if h > 0 and w > 0:
                    ...
                    radius = self.opt.hm_gauss if self.opt['mse_loss'] is True else radius
                    # print('h, w: ', h, w)
                    # print('radius: ', radius)
                    # if self.opt.polygon:
    ...
    def __getitem__(self, index):
        ...
        hm_i, wh_i, dense_wh_i, reg_mask_i, ind_i, cat_spec_wh_i, cat_spec_mask_i, reg_i, gt_dets_i, cat_depth = self.load_sample(
            sample, img_dir, num_images=self.num_agents)
        if self.opt['coord'] == 'Joint':
            ret = {'input': aug_imgs, 'trans_mats': trans_mats, 'trans_mats_n005': trans_mats_n005,
                   'trans_mats_n010': trans_mats_n010, 'trans_mats_p005': trans_mats_p005, \
                   'trans_mats_p007': trans_mats_p007, 'trans_mats_p010': trans_mats_p010,
                   'trans_mats_p015': trans_mats_p015, 'trans_mats_p020': trans_mats_p020, \
                   ...
                   'cat_depth': cat_depth, \
                   'shift_mats_1': shift_mats_1, 'shift_mats_2': shift_mats_2, 'shift_mats_4': shift_mats_4,
                   'shift_mats_8': shift_mats_8}
        ...
        if 'dense_wh' in self.opt:
            ...
            del ret['wh']
            del ret['wh_i']
        if self.opt['reg_offset'] is True:
            if self.opt['coord'] == 'Global':
                ret.update({'reg': reg})
            elif self.opt['coord'] == 'Local':
        ...
        if 'debug' in self.opt or not self.split == 'train':
