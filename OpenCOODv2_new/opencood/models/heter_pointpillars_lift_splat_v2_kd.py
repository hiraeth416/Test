...
class HeterPointPillarsLiftSplatV2KD(nn.Module):
    def __init__(self, args):
        super(HeterPointPillarsLiftSplatV2KD, self).__init__()
    ...
    def forward(self, data_dict):
        ...
        """
        Get foreground feature
        """
        if not skip_camera and not skip_lidar:
            foreground_mask = F.sigmoid(cls_preds_before_fusion.detach()).max(dim=1)[0] > 0.15  # [N, H, W]

            lidar_foreground_mask = foreground_mask[lidar_agent_indicator==1] # [N_L, H, W]
            camera_foregroud_mask = foreground_mask[lidar_agent_indicator==0] # [N_C, H, W]
            lidar_feature_agg = lidar_feature_2d.permute(0,2,3,1)[lidar_foreground_mask==1]
            camera_feature_agg = camera_feature_2d.permute(0,2,3,1)[camera_foregroud_mask==1]
        ...
        if not skip_camera and not skip_lidar:
            output_dict = {'cls_preds_single': cls_preds_before_fusion, 
                        ...
                        'reg_preds': reg_preds,
                        'dir_preds': dir_preds,
                        'lidar_feature_agg': lidar_feature_agg,
                        'camera_feature_agg': camera_feature_agg}
        else:
            output_dict = {'cls_preds_single': cls_preds_before_fusion, 
                        'reg_preds_single': reg_preds_before_fusion, 
                        'dir_preds_single': dir_preds_before_fusion, 
