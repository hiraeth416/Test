...
class V2XSIMBaseDataset(Dataset):
    ...
    def get_len_record(self):
        len_record = []
        timestamps = [x['timestamp'] for x in self.dataset_info_pkl]
        t_0 = -1
        cur_timestamps = []
        for t_i in timestamps:
            if t_i > t_0:
                cur_timestamps.append(t_i)
            else:
                if len(len_record) == 0:
                    len_record.append(len(cur_timestamps))
                else:
                    prev_last = len_record[-1]
                    len_record.append(prev_last + len(cur_timestamps))
                cur_timestamps = [t_i]
            t_0 = t_i
        return len_record
    ...
    def reinitialize(self):
        ...
        if self.ego_mode == 'one':
            ...
            self.len_record = self.get_len_record()
        else:
    ...
    def retrieve_base_data(self, idx):
        ...
        for cav_id, cav_content in scene.items():
            ...
            scan = np.fromfile(cav_content['lidar'].replace('dataset', '/remote-home/share'), dtype='float32')
