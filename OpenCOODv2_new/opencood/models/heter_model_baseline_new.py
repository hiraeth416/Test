...
class HeterModelBaselineNew(nn.Module):
    def __init__(self, args):
        super(HeterModelBaselineNew, self).__init__()
        self.args = args
        ...
        for i in range(len(args['fusion_backbone']['layer_nums'])):
            ...
            #    self.fusion_net.append(AttFusion(args['att']['feat_dim'][i]))
                ...
                self.fusion_net.append(AttFusion(64))
            elif args['fusion_method'] == "disconet":
                self.fusion_net.append(DiscoFusion(64))
            elif args['fusion_method'] == "v2vnet": 
                self.fusion_net.append(V2VNetFusion(args['v2vnet']))
            elif args['fusion_method'] == "v2xvit":
                self.fusion_net.append(V2XViTFusion(args['v2xvit']))
            elif args['fusion_method'] == "when2comm":
                self.fusion_net.append(When2commFusion(args['when2comm']))
            elif args['fusion_method'] == "where2comm":
                self.fusion_net.append(Where2commFusion(args['where2comm']))
            else:
    ...
    def forward(self, data_dict):
        ...
        """
        print("heter_feature_2d: ", heter_feature_2d.size())
        feature_list = [heter_feature_2d]
        #print(heter_feature_2d.size())
        ...
        print(communication_masks[0].size())
