...
class HeterModelSharedheadLate(nn.Module):
    def __init__(self, args):
        super(HeterModelSharedheadLate, self).__init__()
        self.args = args
    ...
    def forward(self, data_dict):
        ...
        for modality_name in self.modality_name_list:
            ...
            #print("feature_first:", feature.shape)
            feature = eval(f"self.backbone_{modality_name}")({"spatial_features": feature})['spatial_features_2d']
            #print("feature_second:", feature.shape)
            feature = eval(f"self.aligner_{modality_name}")(feature)
            #print("feature_third:", feature.shape)
        ...
        heter_feature_2d = self.backbone.decode_multiscale_feature(heter_feature_2d_list)
        '''
        '''
        if self.shrink_flag:
            heter_feature_2d = self.shrink_conv(heter_feature_2d)
        '''
        ...
        for i, fuse_module in enumerate(self.fusion_net):
            fused_feature_list.append(fuse_module(feature_list[i], record_len, t_matrix))
        feature = self.backbone.decode_multiscale_feature(fused_feature_list)
