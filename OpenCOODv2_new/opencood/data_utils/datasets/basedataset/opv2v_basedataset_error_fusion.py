...
class OPV2VBaseDataset(Dataset):
    def __init__(self, params, visualize, train=True):
        ...
        self.scenario_ego_ids = OrderedDict()
        self.reinitialize()
    ...
    def reinitialize(self):
        ...
        for (i, scenario_folder) in enumerate(self.scenario_folders):
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                print("len of timestamp of {}: ".format(cav_id), len(timestamps))
                
                if j == 0: # ego
                    timestamps = timestamps[self.tau:]        # 从第k个往后
                else:
                    timestamps = timestamps[:-self.tau]       # 丢掉最后k个 
                ...
                if j == 0:
                    ...
                    self.scenario_ego_ids[i] = cav_id
    ...
    def retrieve_base_data(self, idx):
        ...
        # print("scenario_database cav data :", scenario_database[0][timestamp_index])
        data = OrderedDict()
        # load files for all CAVs
        for cav_id, cav_content in scenario_database.items():
            # print("timestamp_index", timestamp_index)
            # print("timestamp_key", list(cav_content.items())[timestamp_index])
            # print the len of cav_content
            print("len(cav_content) of {}".format(cav_id), len(cav_content))
            timestamp_key = list(cav_content.items())[timestamp_index][0]
            if cav_id == self.scenario_ego_ids[scenario_index]:
                data[cav_id] = OrderedDict()
                data[cav_id]['ego'] = True
            else:
                data[cav_id] = OrderedDict()
                data[cav_id]['ego'] = False
            # print(list(cav_content.items())[timestamp_index])
