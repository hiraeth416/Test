...
class BasePostprocessor(object):
    ...
    def generate_gt_bbx(self, data_dict, return_idx=False):
        """
        ...
        if return_idx:
            return gt_box3d_tensor, gt_object_idx
        else:
            return gt_box3d_tensor

    def generate_previous_timestamp_gt_box(self, data_dict, return_idx=False, previous_timestamp=0):
        # previous_timestamp: 0 means the current timestamp, 1 means the previous timestamp

        data_dict = data_dict['ego']['prev_info'][previous_timestamp]
        ...
        gt_box3d_np, mask = box_utils.mask_boxes_outside_range_numpy(gt_box3d_np,
                                                                     ...
                                                                     order=None, return_mask=True)
        gt_box3d_tensor = torch.from_numpy(gt_box3d_np).to(device=gt_box3d_list[0].device)
        gt_object_idx = np.array(list(set(object_id_list)))[np.where(mask)[0]]
    ...
    def generate_object_center_v2xseq(self,
                               cav_contents,
                               reference_lidar_pose):
        """
        Retrieve all objects in a format of (n, 7), where 7 represents
        ...
        box_utils.project_lidar_objects_v2xseq(tmp_object_list,
                                        output_dict,
                                        reference_lidar_pose,
                                        filter_range,
                                        self.params['order'])
