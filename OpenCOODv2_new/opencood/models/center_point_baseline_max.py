...
class CenterPointBaselineMax(nn.Module):
    """
    F-Cooper implementation with point pillar backbone.
    """
    def __init__(self, args):
        super(CenterPointBaselineMax, self).__init__()
    ...
    def forward(self, data_dict):
        ...
        # fused_feature = self.fusion_net(batch_dict['spatial_features'], record_len, t_matrix)
        # batch_dict['spatial_features'] = fused_feature
        ...
        for i, _ in enumerate(feature_list):
            if i == fusion_layer_id:
                feat_fused, neighbor_features = self.fusion_net(feature_list[i], record_len, t_matrix)
                fused_feature_list.append(feat_fused)
            else:
                split_feats = self.regroup(feature_list[i], record_len)
                ...
                fused_feature_list.append(curr_ego_feat)
        ...
        if DEBUG:
            ...
            save_dir = '/remote-home/share/yhu/Co_Flow/opencood/visualization/vis_feat_max/{}.png'.format(self.vis_count)
            N = neighbor_features.shape[0]
            fig, axes = plt.subplots(N+1, 1)
            # for ax_i, cur_feat in enumerate(split_feat_list_single[0][:,0]):
            for ax_i, cur_feat in enumerate(neighbor_features):
                ...
                axes[ax_i].set_title('agent_{}'.format(ax_i))
