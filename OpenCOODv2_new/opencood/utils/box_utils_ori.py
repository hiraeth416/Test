...
def project_world_objects_v2xseq(object_list,
                          output_dict,
                          lidar_pose,
                          lidar_range,
                          order):
    ...
    for object_content in object_list:
        ...
        x, y, z = object_content['3d_location'].values()
        h, w, l = object_content['3d_dimensions'].values()
        rot = object_content['rotation']
        box = np.array([x, y, z, h, w, l, rot])[np.newaxis, :]

        corners_world = boxes_to_corners_3d(box, order='hwl')[0]
        corners_world_homo = np.pad(corners_world, ((0,0), (0,1)), constant_values=1) # [8, 4]
        corners_lidar = (world_to_lidar @ corners_world_homo.T).T
...
def project_lidar_objects_v2xseq(object_list,
                          output_dict,
                          lidar_pose,
                          lidar_range,
                          order):
    """
    objects lidar coordinates
    ...
    for object_content in object_list:
        ...
        x, y, z = object_content['3d_location']['x'], object_content['3d_location']['y'], object_content['3d_location']['z']
        h, w, l = object_content['3d_dimensions']['h'], object_content['3d_dimensions']['w'], object_content['3d_dimensions']['l']
        rot = object_content['rotation']
        box = np.array([x, y, z, h, w, l, rot])[np.newaxis, :]
        ...
        bbx_lidar = mask_boxes_outside_range_numpy(box, lidar_range_z_larger, order)
        if bbx_lidar.shape[0] > 0:
