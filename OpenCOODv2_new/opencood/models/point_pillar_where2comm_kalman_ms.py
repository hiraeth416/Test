...
class PointPillarWhere2commKalman(nn.Module):
    ...
    def fusion(self, feat, conf_map, record_len, pairwise_t_matrix, request_map=None, channel_compression=False,
               config_thre=True, backbone=None):
        '''
    ...
    def iterateKal(self, cur_feat, record_len, pairwise_t_matrix, trans_mats_dict, frame_id, colla_flag=False,
                   fuse_layer_id=0, predict_decay=0.8, prev_feat_box=None, anchors=None, backbone=None):

        # 1. current ego / collaborative results
        psm_single = self.cls_head(self.decode(self.encode(cur_feat)))
        confidence_maps = psm_single.sigmoid().max(dim=1)[0].unsqueeze(
            1)  # dim1=2 represents the confidence of two anchors
        if colla_flag:
            request_maps = None
            feats_colla, comm_rate, fusion_result_dict = self.fusion(cur_feat, confidence_maps, record_len,
                                                                     pairwise_t_matrix, request_maps,
                                                                     channel_compression=False, config_thre=True, backbone=backbone)
            # downsample feature to reduce memory
            if self.shrink_flag:
                feats_colla = self.shrink_conv(feats_colla)
            feats_colla = self.get_ego_feat(feats_colla, record_len)
        else:
            detect_feat = self.get_ego_feat(cur_feat, record_len)
            feats_colla = self.decode(self.encode(detect_feat))
        ...
        box_preds = torch.from_numpy(box_preds).to(cur_feat.device)
        box_previous = torch.from_numpy(box_previous).to(cur_feat.device)
        box_preds_score = torch.from_numpy(box_preds_score).to(cur_feat.device).squeeze()
        ...
        if colla_flag is False and prev_feat_box is not None:
            ...
            if box_previous is not None:
                ...
                flow_map, reserved_mask = self.generate_box_flow_map(box_previous, box_preds, scale=2.5, shape_list=torch.tensor(prev_feat.shape[-3:]).to(prev_feat.device))
                warped_out = self.warp_w_box_flow(prev_feat_masked, flow_map, reserved_mask)
                out_list = self.encode(warped_out)
                feat_list = self.encode(cur_feat[0].unsqueeze(0))
                fused_out = []
                for l_i in range(len(out_list)):
                    fused_out.append(self.hist_fusion_net(feat_list[l_i], out_list[l_i]))
                out = self.decode(fused_out)
            else:
                warped_out = cur_feat[0].unsqueeze(0)
                out = self.decode(self.encode(warped_out))

            fused_feats = self.get_ego_feat(out, record_len)
            ...
            debug_dict.update({'colla_feat': warped_out.clone(),
    ...
    def forward(self, data_dict, dataset=None):
        ...
        comm_rates = []
        past_i = 0
        C, H, W = batch_dict['spatial_features'].shape[-3:]
        # cur_feat_list = [x[:, past_i] for x in split_feat_list_single]
        cur_feat = batch_dict['spatial_features'].view(-1, num_sweep_frames, C, H, W)[:,0]
        cur_pairwise_t_matrix = get_pairwise_transformation_torch(lidar_pose[:, past_i], 5, record_len)
        if self.with_hist:
            fused_box_dict, debug_dict = self.iterateKal(cur_feat, record_len, cur_pairwise_t_matrix,
    ...
    def generate_predicted_boxes(self, cls_preds, deltas, anchors, dir_preds=None):
        ...
        if dir_preds is not None:
            ...
            print('dir_rot: ', dir_rot.shape)
            print('dir_labels: ', dir_labels.shape)
