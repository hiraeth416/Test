...
class PointPillarWhere2commKalmanAllocate(nn.Module):
    def __init__(self, args):
        super(PointPillarWhere2commKalmanAllocate, self).__init__()
    ...
    def get_predictable_flag(self, history, thre=0.8):
        prev_hist = np.array(history[-2])
        cur_hist = np.array(history[-1])
        print('Diff: ', np.abs((cur_hist - prev_hist)[:,-4:]))
        if np.abs((cur_hist - prev_hist)[:,-4:]).max() < thre:
            return 1
        return 0
    ...
    def predictKal(self, device):
        ...
        for t, trk in enumerate(trks):
            ...
            if len(self.MOTtracker.trackers[t].history) >= 3:
                predictable_flag.append(self.get_predictable_flag(self.MOTtracker.trackers[t].history))
            else:
                predictable_flag.append(0)
        ...
        keep_idx = np.where(predictable_flag)[0]
        print('keep_objects/all_objects:', len(keep_idx), len(self.MOTtracker.trackers))
        return trks, score, trk_previous, keep_idx
    ...
    def generate_request_map(self, box, keep_idx, shape_list, scale=1.25):
        bbox_list = torch.tensor(box[:, :4, :2]).to(shape_list.device)
        C, H, W = shape_list
        ...
        # request_map = torch.ones((C, H, W)).to(shape_list.device)  # C, H, W 
        request_map = torch.zeros((C, H, W)).to(shape_list.device)  # C, H, W 
        # generate mask
        ...
        for cav in range(num_cav):
            request_map[:,y_min_fid_ori[cav]:y_max_fid_ori[cav],x_min_fid_ori[cav]:x_max_fid_ori[cav]] = 1
    ...
    def iterateKal(self, cur_feat_list, record_len, pairwise_t_matrix, trans_mats_dict, frame_id, colla_flag=False,
        ...
        # if self.comm_thre <= 0.03:
        #     colla_flag = True

        # 2. kalman filter prediction result
        box_preds, box_preds_score, box_previous, keep_idx = self.predictKal(feats.device)  # shape [N, 8, 3]
        ...
        if colla_flag:
            ...
            scale = 1
            _, H, W = feats.shape[-3:]
            ego_request_map = self.generate_request_map(box_preds, keep_idx, scale=1.25*scale, shape_list=torch.tensor([1, H*scale, W*scale]).to(feats.device))
            request_maps = torch.ones_like(confidence_maps).to(confidence_maps.device)
            request_maps[0,0] = ego_request_map
        ...
        box_colla, box_colla_score = self.feature2box(cls, bbox_temp,
                                                      trans_mats_dict['trans_mats_track'],
                                                      cav_lidar_range=self.cav_lidar_range)  # shape [N, 8, 3]
        ...
        if prev_feat_box is not None:
            ...
            # prev_feat_masked = torch.gt(prev_conf, 0.00001) * prev_feat
            # prev_feat_masked = torch.gt(prev_conf, 0.001) * prev_feat
            ...
            if box_previous is not None:
                ...
                flow_map, reserved_mask = self.generate_box_flow_map(box_previous[keep_idx], box_preds[keep_idx], scale=1.25*scale, shape_list=torch.tensor([C, H*scale, W*scale]).to(prev_feat.device))
                ...
                # out = self.hist_fusion_net(out, feats[0].unsqueeze(0), confidence_maps[0].unsqueeze(0), 0.0)
                out = self.hist_fusion_net(out, feat_list_colla[0], cls.sigmoid().max(dim=1)[0].unsqueeze(1), 0.0)
            ...
            bbox_temp = self.generate_predicted_boxes(cls, bbox, anchors, dir_preds)
            fused_feat_box, fused_feat_box_score = self.feature2box(cls, bbox_temp, trans_mats_dict['trans_mats_track'], cav_lidar_range=self.cav_lidar_range)
    ...
    @staticmethod
    def feature2box(cls_preds, reg_preds, transformation_matrix, score_thre=0.2, cav_lidar_range=[-140.8, -38.4, -3, 140.8, 38.4, 1]):
        pred_box3d_list = []
        pred_box2d_list = []
