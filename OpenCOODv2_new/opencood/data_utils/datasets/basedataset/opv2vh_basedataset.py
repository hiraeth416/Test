...
class OPV2VHBaseDataset(Dataset):
    def __init__(self, params, visualize, train=True):
        print("Initialize OPV2VHBaseDataset")
        self.params = params
        ...
        if 'test_lidar_dir' in params and not self.train:
            self.lidar_root_dir = params['test_lidar_dir']
        else:
            self.lidar_root_dir = self.root_dir
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                for timestamp in timestamps:
                    ...
                    yaml_file = os.path.join(cav_path,
                                             timestamp + '.yaml')
                    lidar_cav_path = cav_path.replace(self.root_dir, self.lidar_root_dir)
                    # print("self.root_dir: ", self.root_dir)
                    # lidar_cav_path = self.root_dir
                    # print("lidar_root_dir: ", self.lidar_root_dir)
                    lidar_file = os.path.join(lidar_cav_path,
                                              timestamp + '.pcd')
                ...
                if j == 0:
                    ...
                    num_ego_timestamps = len(timestamps) - (self.tau + 1 - 1)	
    ...
    def retrieve_base_data(self, idx):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            if curr_timestamp_key not in cav_content or delayed_timestamp_key not in cav_content:
                lack_agent_list.append(cav_id)
                continue
            if cav_id in lack_agent_list:
                continue
            ...
            # load lidar yaml file
            lidar_yaml_file = cav_content[curr_timestamp_key]['yaml'].replace(self.root_dir, self.lidar_root_dir)
            data_vehicle = load_yaml(lidar_yaml_file)
            data[cav_id]['params']['vehicles']=data_vehicle['vehicles']
            ...
            if self.load_lidar_file or self.visualize:
                data[cav_id]['lidar_np'] = \
                    pcd_utils.pcd_to_np(cav_content[input_timestamp_key]['lidar'].replace(\
                        self.root_dir, self.lidar_root_dir))

            if getattr(self, "heterogeneous", False):
                data[cav_id]['modality_name'] = cav_content[curr_timestamp_key]['modality_name']
