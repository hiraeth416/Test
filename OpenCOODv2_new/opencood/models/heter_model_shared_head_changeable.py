...
class HeterModelSharedheadMultiPlus(nn.Module):
    def __init__(self, args):
        super(HeterModelSharedheadMultiPlus, self).__init__()
        self.args = args
        ...
        self.multi_channel_compressor = []
        for residual in range(args['codebook']['r']):
            self.multi_channel_compressor.append(UMGMQuantizer(channel, seg_num, dict_size, p_rate,
        ...
        self.multi_channel_compressor=nn.ModuleList(self.multi_channel_compressor)
    ...
    def freeze_codebook(self):
        for compressor in self.multi_channel_compressor:
    ...
    def forward(self, data_dict):
        ...
        if self.multi_channel_compressor_flag:
            ...
            codebook_loss=0
            heter_feature_2d_old =torch.zeros_like(heter_feature_2d)
            heter_feature_2d_list = []
            for compressor in self.multi_channel_compressor:    
                heter_feature_2d_new, _, _, codebook_loss_temp = compressor(heter_feature_2d - heter_feature_2d_old)
                
                heter_feature_2d_old = heter_feature_2d_new+heter_feature_2d_old
                heter_feature_2d_list.append(heter_feature_2d_old)
                codebook_loss+=codebook_loss_temp
            if self.istrain:
                print("training")
                random_index = random.randint(0, len(heter_feature_2d_list)-1)
            else:
                random_index = -1
                
            heter_feature_2d = heter_feature_2d_list[random_index].view(-1, H, W, C).permute(0, 3, 1, 2).contiguous()
