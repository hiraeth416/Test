...
from opencood.models.fuse_modules.where2comm1 import Where2comm
...
class PointPillarWhere2commKalmanAllocateLatency(nn.Module):
    def __init__(self, args):
        super(PointPillarWhere2commKalmanAllocateLatency, self).__init__()
        ...
        if 'temporal_args' in args and 'latency' in args['temporal_args']:
            self.latency = args['temporal_args']['latency']
        if 'temporal_args' in args and 'fix_latency' in args['temporal_args']:
            self.fix_latency = args['temporal_args']['fix_latency']
    ...
    def updateKal(self, dets, dets_score, trks, MOTtracker):
        dets = corner_to_center(dets)  # [xyz, l, w, h, theta]
        dets_score = dets_score.unsqueeze(1)
        trks = corner_to_center(trks)  # --> [xyz, l, w, h, theta]
        trks = trks[:, MOTtracker.reorder]  # -->[x, y, z, theta, l, w, h]

        # dets: [N, 7]
        dets = dets[:, MOTtracker.reorder]  # before [x, y, z, l, w, h, theta] after: [x, y, z, theta, l, w, h]
        info = dets_score
        ...
        trks_S = [np.matmul(np.matmul(tracker.kf.H, tracker.kf.P), tracker.kf.H.T) + tracker.kf.R for tracker in MOTtracker.trackers]   # H*P*H' + R
        ...
        for t, trk in enumerate(MOTtracker.trackers):
        ...
        for i in unmatched_dets:  # a scalar of index
            ...
            trk = KalmanBoxTracker(dets[i, :], info[i, :], dets_8corner[i, :], MOTtracker.covariance_id,
                                   track_score,
                                   MOTtracker.tracking_name, MOTtracker.use_angular_velocity)
            MOTtracker.trackers.append(trk)
            ids[i, 0] = trk.id
        # create_time = time.time()
        # print('create time: ', create_time - update_time)

        i = len(MOTtracker.trackers)
        for trk in reversed(MOTtracker.trackers):

            d = trk.get_state()  # bbox location
            d = d[MOTtracker.reorder_back]
            ...
            if trk.time_since_update >= MOTtracker.max_age:
                MOTtracker.trackers.pop(i)
    ...
    def resetKal(self):
        ...
        self.MOTtracker_hist.trackers = []
        self.MOTtracker_hist.frame_count = 0
        self.prev_feat_box_dict = None

    def getKal(self, MOTtracker, past_length=0):
        trk_previous = np.zeros((len(MOTtracker.trackers), 7))  # N x 7 , #get previous locations from trackers.
        
        for t, trk in enumerate(trk_previous):
            # pos = MOTtracker.trackers[t].get_state().reshape((-1, 1))
            if len(MOTtracker.trackers[t].history) > past_length:
                pos = np.concatenate(MOTtracker.trackers[t].history, axis=-1).T[-past_length].reshape((-1, 1))
                trk_previous[t] = [pos[0], pos[1], pos[2], pos[3], pos[4], pos[5], pos[6]]
        trk_previous = trk_previous[:, MOTtracker.reorder_back]
        trk_previous = box_utils.boxes_to_corners_3d(trk_previous, order='lwh')
        return trk_previous

    def predictKal(self, MOTtracker):
        trks = np.zeros((len(MOTtracker.trackers), 7))  # N x 7 , #get predicted locations from existing trackers.
        score = np.zeros((len(MOTtracker.trackers), 1))
        to_del = []
        for t, trk in enumerate(trks):
            pos = MOTtracker.trackers[t].predict().reshape((-1, 1))
            trk[:] = [pos[0], pos[1], pos[2], pos[3], pos[4], pos[5], pos[6]]
            score[t] = MOTtracker.trackers[t].track_score.detach().cpu().numpy()
            if (pd.isnull(np.isnan(pos.all()))):  # ly
                to_del.append(t)
        trks = np.ma.compress_rows(np.ma.masked_invalid(trks))
        score = np.ma.compress_rows(np.ma.masked_invalid(score))
        for t in reversed(to_del):
            MOTtracker.trackers.pop(t)

        # trks: x,y,z,l,w,h,theta
        trks = trks[:, MOTtracker.reorder_back]
    ...
    def generate_colla_memory(self, cur_feat_list, pairwise_t_matrix, record_len, trans_mats_dict, anchors, fuse_layer_id=0, frame_id=0, predict_decay=0.8):
        '''
            Save the collaborator's features at each historical timestamp
            The start the collabortion when it reaches the latency time
        '''
        ...
        confidence_maps_colla = cls.sigmoid().max(dim=1)[0].unsqueeze(1)
        ...
        box_previous_npy = self.getKal(self.MOTtracker_hist, past_length=self.latency)
        box_preds_npy, box_preds_score_npy = self.predictKal(self.MOTtracker_hist)  # shape [N, 8, 3]

        box_preds = torch.from_numpy(box_preds_npy).to(feats.device)
        box_preds_score = torch.from_numpy(box_preds_score_npy).to(feats.device).squeeze()
        ...
        self.updateKal(box_update_global, box_update_score, box_preds_global, self.MOTtracker_hist)
        ...
        debug_dict = {
            ...
            'reg_preds': bbox,
            'dir_preds': dir_preds,
            'box_previous_npy': box_previous_npy,
            'box_preds_npy': box_preds_npy,
            'box_preds_score_npy': box_preds_score_npy
        }
        return debug_dict
    ...
    def iterateKal(self, cur_feat_list, record_len, pairwise_t_matrix, trans_mats_dict, frame_id, colla_flag=False,
        ...
        ####### 0. process memory; save the perfect collaboration in the memory #########
        hist_dict = self.generate_colla_memory(cur_feat_list, pairwise_t_matrix, record_len, trans_mats_dict, anchors)
        
        # 1. current ego / collaborative results
        feat_list_colla = []
        for layer_id, feat in enumerate(cur_feat_list):
            feat_list_colla.append(self.get_ego_feat(feat, record_len))
        feats = self.decode(feat_list_colla)
        cls = self.cls_head(feats)
        bbox = self.reg_head(feats)
        confidence_maps_colla = cls.sigmoid().max(dim=1)[0].unsqueeze(1)
        dir_preds = None
        if self.use_dir:
            dir_preds = self.dir_head(feats)
        bbox_temp = self.generate_predicted_boxes(cls, bbox, anchors, dir_preds)

        box_ego, box_ego_score = self.feature2box(cls, bbox_temp,
                                                      trans_mats_dict['trans_mats_track'])  # shape [N, 8, 3]
        
        if self.latency == 0:
            output_box_dict = {'fused_box': hist_dict['box_colla'],
                            'fused_box_score': hist_dict['box_colla_score'],
                            'fused_feat_box': box_ego,
                            'fused_feat_box_score': box_ego_score,
                            'comm_rate': torch.zeros(0).to(feats.device)
                            }
            return output_box_dict, hist_dict, hist_dict
        else:
            output_box_dict = {'fused_box': box_ego,
                            'fused_box_score': box_ego_score,
                            'fused_feat_box': box_ego,
                            'fused_feat_box_score': box_ego_score,
                            'comm_rate': torch.zeros(0).to(feats.device)
                            }
            debug_dict = hist_dict
            box_update = box_ego
            box_update_score = box_ego_score
            if len(self.memory_bank)>=self.latency:
                # 2. load memory and compensate to current timestamp
                box_preds, box_preds_score, box_previous = hist_dict['box_preds_npy'], hist_dict['box_preds_score_npy'], hist_dict['box_previous_npy']
                ...
                if box_previous is not None:
                    lidar_pose = torch.cat([trans_mats_dict['lidar_pose'][0],
                                            self.memory_bank[0]['lidar_pose'][0]], dim=0)
                    prev_feat = self.warp_feat(self.memory_bank[0]['colla_feat'][0][None, ...], lidar_pose)
                    prev_conf = self.warp_feat(self.memory_bank[0]['colla_conf'][0][None, ...], lidar_pose)
                    ...
                    if box_previous is not None:
                        ...
                        out = self.hist_fusion_net(out, cur_feat_list[0][0].unsqueeze(0), confidence_maps_colla[0].unsqueeze(0), 0.01)
                    else:
                        out = cur_feat_list[0][0].unsqueeze(0)
                    ...
                    debug_dict = {'colla_feat': out.clone(),
                                    'colla_conf': cls.sigmoid().max(dim=1)[0].unsqueeze(1).clone(),
                    ...
                    output_box_dict.update({'fused_box': box_update,
                                    'fused_box_score': box_update_score,
                                    'fused_feat_box': fused_feat_box,
                                    'fused_feat_box_score': fused_feat_box_score,
                                    'comm_rate': torch.zeros(0).to(out.device)
                                    })
            # 4. kalman filter update
            box_preds, box_preds_score = self.predictKal(self.MOTtracker)
            ...
            box_update_global = box_utils.project_box3d(box_update, trans_mats_dict['ego2world_list'][frame_id]).cpu().numpy()
            box_preds_global = box_utils.project_box3d(box_preds, trans_mats_dict['ego2world_list'][frame_id]).cpu().numpy()
            self.updateKal(box_update_global, box_update_score, box_preds_global, self.MOTtracker)
            return output_box_dict, debug_dict, hist_dict
    ...
    def forward(self, data_dict, dataset=None):
        ...
        if self.with_hist:
            fused_box_dict, debug_dict, hist_dict = self.iterateKal(cur_feat_list, record_len, cur_pairwise_t_matrix,
        ...
        hist_dict.update({'lidar_pose': lidar_pose})
        self.memory_bank.append(hist_dict)
        # print('memory_bank: ', len(self.memory_bank))
