...
def main():
    ...
    model = train_utils.create_model(hypes) #, thre=opt.thre)
    ...
    loaded_pretrained_codebook = False
    if loaded_pretrained_codebook:
        # cb_path = "opencood/logs/cb_only/v2xsimmodel_4.pth"
        cb_path = "opencood/logs/cb_only/model_6.pth"
        resume_epoch, model = train_utils.load_saved_model(saved_path, model, pretrained_path=cb_path)
    ...
    # [14, 40, 99, 145, 80, 154, 59, 20, 179, 198, 207, 144, 163, 167, 101, 210]
    class Mysubset(torch.utils.data.Subset):
        def retrieve_scene_idx(self, idx):
            len_record = [14, 40, 99, 145, 80, 154, 59, 20, 179, 198, 207, 144, 163, 167, 101, 210]
            scenario_index = 0
    ...
    # opencood_dataset_subset = Subset(opencood_dataset, range(1870, 1880))
    # opencood_dataset_subset = Mysubset(opencood_dataset, range(1870, 1880))
        ...
        with torch.no_grad():
            ...
            if opt.fusion_method == 'late':
                ...
                infer_result = infer_func(batch_data,
                                          model,
                                          opencood_dataset)
                # comm_rate = infer_result['comm_rate'] * 10000
                # print(f"comm_rate: {comm_rate}")
    ...
    # total_avg_comm_rate = sum(comm_rate_list) / len(comm_rate_list)
    # print('total_avg_comm_rate: ', total_avg_comm_rate)
