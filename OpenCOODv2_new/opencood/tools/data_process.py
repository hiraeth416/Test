...
def to_npy_and_json(split_name="train"):
    ...
    # pcd / yaml 文件路径
    root_dir = os.path.join("/remote-home/share/v2v4real", data_seperate_name)
    # 新保存的路径
    save_dir = os.path.join("/remote-home/share/v2v4real", data_seperate_name)
...
def export_legend(ax, filename="legend.pdf"):
    ...
    ax2.axis('off')
    legend = ax2.legend(*ax.get_legend_handles_labels(), frameon=False, loc='lower center', ncol=4,fontsize='xx-large')
    fig  = legend.figure
    fig.canvas.draw()
...
if __name__ == "__main__":
    to_npy_and_json(split_name='train')
    ...
    for split_i, split_name in enumerate(split_list):
        ...
        plt.sca(ax50); plt.plot(delays, ap_50_list, color=color, linewidth='5', label = method_name)#, marker='+') # , marker='+'
