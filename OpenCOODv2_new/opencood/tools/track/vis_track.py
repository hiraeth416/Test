...
def visualize(pred_box_np, gt_box_np, pcd_np, save_path, method='bev', left_hand=False):
    """
    ...
    if pred_box_np is not None:
        pred_name = [str(int(x)) for x in pred_box_np[:, 1]]
    ...
    if pred_box_np is not None:
        ...
        pred_corners = boxes_to_corners_3d(pred_box_np[:, 2:], order='lwh')
        ...
        canvas.draw_boxes(pred_corners, colors=(0, 0, 255), texts=pred_name)
...
if __name__ == "__main__":
    ...
    for scene in scenes:
        ...
        for frame in tqdm(det_seq):
            ...
            visualize(det_box, gt_box, pcd_np, image_save_path, method='bev', left_hand=False)
