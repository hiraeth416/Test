...
def getIntermediateheterFusionDataset(cls):
    ...
    class IntermediateheterFusionDataset(cls):
        ...
        def __getitem__(self, idx):
            ...
            if len(cav_id_list) < self.min_cav_num:
                return None
            else:
                if len(cav_id_list) > self.max_cav_num:
                  random_exclude_idx = random.sample(range(1,len(cav_id_list)), len(cav_id_list) - self.max_cav_num)
                  for idx in random_exclude_idx:
                      exclude_agent.append(cav_id_list[idx])
                  new_cav_id_list = []
                  new_lidar_pose_list = []
                  new_lidar_pose_clean_list = []
                  for i in range(len(cav_id_list)):
                      if cav_id_list[i] in exclude_agent:
                          continue
                      else:
                          new_cav_id_list.append(cav_id_list[i])
                          new_lidar_pose_list.append(lidar_pose_list[i])
                          new_lidar_pose_clean_list.append(lidar_pose_clean_list[i])
                  cav_id_list = new_cav_id_list
                  lidar_pose_list = new_lidar_pose_list
                  lidar_pose_clean_list = new_lidar_pose_clean_list
