...
class CenterPointWhere2commMultiSweep(nn.Module):
    def __init__(self, args):
        super(CenterPointWhere2commMultiSweep, self).__init__()
        ...
        self.temporal_fusion_net = MaxFusion(args['fusion_args'])
    ...
    def forward(self, data_dict):
        ...
        fused_feature = self.temporal_fusion_net(fused_features, frame_record_len, cur_pairwise_t_matrix)
        ...
        if DEBUG:
            ...
            fig, axes = plt.subplots(len(fused_feature_list)+1, 1)
            for ax_i, cur_feat in enumerate(fused_feature_list):
                cur_feat = fused_feature_list[ax_i][0].max(dim=1)[0].cpu().numpy()[0]
