...
class CenterPointWhere2commMultiSweepFlow(nn.Module):
    def __init__(self, args):
        ...
        print('temporal_thre: ', self.temporal_thre)
        ...
        # self.cls_head_whist = nn.Conv2d(self.out_channel, args['anchor_number'],
        #                           kernel_size=1)
        # self.reg_head_whist = nn.Conv2d(self.out_channel, 8 * args['anchor_number'],
        #                           kernel_size=1)
        
        # self.cls_head_onlyhist = nn.Conv2d(self.out_channel, args['anchor_number'],
        #                           kernel_size=1)
        # self.reg_head_onlyhist = nn.Conv2d(self.out_channel, 8 * args['anchor_number'],
        #                           kernel_size=1)
    ...
    def temporal_fusion(self, cur_feat, hist_feat, confidence_maps_hist=None, thre=0.0):
        '''
        ...
        fused_feat = self.hist_fusion_net(cur_feat, hist_feat, confidence_maps_hist, thre)
        return fused_feat
    ...
    def iterate(self, cur_feat_list, lidar_pose, record_len, pairwise_t_matrix, prev_confidence_maps=None, hist_fused_feats=None, flow_gt=None, fuse_layer_id=0):
        ...
        if hist_fused_feats is not None:
            ...
            if self.only_hist:
                ...
                feats_hist = self.decode(self.get_colla_feat_list(cur_feat_list, hist_feats))   # [sum(cav_num)*num_sweep_frames, 256, 100, 352]
                psm_hist = self.cls_head(feats_hist)
                confidence_maps_hist = psm_hist.sigmoid().max(dim=1)[0].unsqueeze(1) # dim1=2 represents the confidence of two anchors
                feats = self.temporal_fusion(curr_feats, hist_feats, confidence_maps_hist, thre=self.temporal_thre)
            else:
                feats = curr_feats
        ...
        if (hist_fused_feats is not None) and self.wo_colla:
            fused_feat = feats
    ...
    def forward(self, data_dict):
        ...
        for past_i in range(num_sweep_frames)[::-1]:
            ...
            if len(hist_fused_feats) >= hist_len:
                fused_feat, comm_rate, confidence_maps_single, confidence_maps, flow, state_class_pred = self.iterate(cur_feat_list, lidar_pose_list, record_len, cur_pairwise_t_matrix, prev_confidence_maps, hist_fused_feats, flow_gt=data_dict['flow_gt'])
                ...
                fused_feat, comm_rate, confidence_maps_single, confidence_maps, flow, state_class_pred = self.iterate(cur_feat_list, lidar_pose_list, record_len, cur_pairwise_t_matrix)
                ego_flow = None
        ...
        for _i, _fused_feat in enumerate(fused_feat):
            ...
            # if _i == 1:
            #     cls = self.cls_head_onlyhist(feats_colla)
            #     bbox = self.reg_head_onlyhist(feats_colla)
            # else:
            #     cls = self.cls_head_whist(feats_colla)
            #     bbox = self.reg_head_whist(feats_colla)
