...
def main():
    ...
    for use_laplace in use_laplace_options:
        ...
        for (pos_mean, pos_std, rot_mean, rot_std) in zip(pos_mean_list, pos_std_list, rot_mean_list, rot_std_list):
            ...
            infer_info = opt.model_name + '_' + opt.fusion_method + '_' + opt.modal + '_' + opt.note + '_noise_' +str(pos_std)
            ...
            with open(os.path.join(opt.model_dir,'detection_noise', '{}_noise_updated.txt'.format(opt.model_name)), 'a+') as f:
